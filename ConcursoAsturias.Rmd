...
head(granja)
str(granja)
aggregate(Lluvia_Tot ~ year, granja, sum)

#sobre todo
todo<-rename(todo, date = TIMESTAMP, ws = VV_media, wd = DV_media,vmax= Vel_Viento_Max)
todo$year<-year(todo$date)
llanual<-aggregate(Lluvia_Tot ~ year+estacion, todo, sum)

library(ggplot2)
ggplot(llanual, aes(x=year, fill=estacion)) +
...
ggplot(llanual,aes(x=year,y=Lluvia_Tot, fill=estacion)) +
...
names(granja)[1]<-"date"
granja$Lluvia_Tot<-granja$Lluvia_Tot*10
max(granja$Lluvia_Tot)
summaryPlot(select(granja, c(date,Vel_Viento_Max, Lluvia_Tot,Temp_Aire_Avg)))
```
